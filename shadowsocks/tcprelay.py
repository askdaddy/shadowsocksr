...
class TCPRelayHandler(object):
    def __init__(self, server, fd_to_handlers, loop, local_sock, config,
        ...
        self._user_name = common.to_bytes(server._user_name)
    ...
    def _handle_stage_addr(self, ogn_data, data):
        try:
            ...
            if connecttype != 0:
                ...
                if common.to_str(remote_addr) not in ['8.8.8.8', '8.8.4.4']:
                    # common.connect_log('TCP request %s:%d by user %d' %
                    #     (common.to_str(remote_addr), remote_port, self._user_id))
                    common.connect_log('TCP request %s:%d by user %s' %
                                       (common.to_str(remote_addr), remote_port, str(self._user_name)))
